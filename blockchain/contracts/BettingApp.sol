...
contract BettingApp {
    ...
    event BetPlaced(uint256 indexed betIndex, address indexed player, uint256 amount);
    event BetResolved(uint256 indexed betIndex, address indexed player, uint256 amount, bool isCheckedOut);
    ...
    function getBet(uint256 betIndex) external view returns (address player, uint256 amount, bool isResolved, bool isCheckedOut) {
        require(betIndex < bets.length, "Invalid bet index");
        Bet storage bet = bets[betIndex];
    ...
    function getMyLatestActiveBet() external view returns (uint256 betIndex, uint256 amount, bool isResolved, bool isCheckedOut) {
